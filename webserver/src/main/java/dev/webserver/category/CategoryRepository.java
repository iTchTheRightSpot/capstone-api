...
public interface CategoryRepository extends CrudRepository<Category, Long> {
    ...
    @Query("SELECT COUNT(category_id) FROM category WHERE parent_id = :parentId")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE category SET name = :name, is_visible = :visible, parent_id = :parentId WHERE category_id = :categoryId")
    void update(
            final String name,
    ...
    (
        ...
        UNION ALL
        SELECT c.category_id FROM rec_category rec INNER JOIN category c ON rec.id = c.parent_id
    )
    UPDATE category c, (SELECT rec.id FROM rec_category rec) AS rec
    SET c.is_visible = FALSE
    WHERE c.category_id = rec.id
    """)
    void updateCategoryAndAllItsChildrenVisibilityToFalse(final long categoryId);
    ...
    SELECT COUNT(DISTINCT p.product_id) FROM rec_category c
    INNER JOIN product p ON p.category_id = c.id
    """)
    int countAllProductsByCategoryIdAdminFront(final long categoryId);
    ...
    WHERE curr.currency = :currency
    GROUP BY p.uuid, p.name, p.default_image_key, curr.currency, curr.price
    ...
    (
        SELECT category_id FROM category
        WHERE category_id = :categoryId AND is_visible = TRUE
        UNION ALL
        SELECT c.category_id FROM rec_category rec
        INNER JOIN category c ON rec.id = c.parent_id
    ...
    INNER JOIN product p ON rec.id = p.category_id
    INNER JOIN product_detail d ON p.product_id = d.product_id
    INNER JOIN product_sku s ON d.detail_id = s.detail_id
    WHERE s.inventory > 0 AND d.is_visible IS TRUE
    """)
    int countAllProductsByCategoryIdWhereInStockAndIsVisible(final long categoryId);
    ...
    SELECT c.* FROM rec_category r
    INNER JOIN category c ON r.id = c.category_id
    """)
