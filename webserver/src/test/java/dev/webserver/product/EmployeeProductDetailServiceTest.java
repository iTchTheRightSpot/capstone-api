...
final class EmployeeProductDetailServiceTest extends AbstractUnitTest {

    private EmployeeProductDetailService detailService;
    ...
    @BeforeEach
    void setUp() {
        detailService = new EmployeeProductDetailService(
                environment,
                detailRepo,
                skuService,
                productRepository,
    ...
    void create() {
        // Given
        final var dtos = TestData.sizeInventoryDTOArray(4);
        final var files = TestData.files();
        final var product = Product.builder().uuid("product uuid").build();
        final var dto = TestData.productDetailDTO(product.uuid(), "mat-black", dtos);
    ...
    void createE() {
        ...
        final var files = TestData.files();
        final var product = Product.builder().uuid("product uuid").build();
        final var detail = ProductDetail.builder().colour(new Faker().commerce().color()).build();
        final var dto = TestData.productDetailDTO(product.uuid(), detail.colour(), dtos);
